...
## 2026-07-15 22:10 | 系统学习课程：骨架与事实边界

### 当前目标
- 面向会 Python 的 RAG 初学者建立代码驱动的中文系统课程
- 先固定课程结构和事实边界，再按调用链逐章补充理论、实验与答案

### 完成内容
- 建立 `docs/learning/` 总索引、课程指南、统一章节模板、术语表和参考资料
- 为 01-15 章建立完整骨架并标注已实现、部分实现、预研三类状态
- 明确快速离线路径与真实模型可选路径，模型缺失时不得伪装验证通过
- 在 `PLAN.md` 增加历史规划警告，防止把 ONNX INT8、真实 KV 复用和生产化写成现状
- 在 `TODO.md` 增加课程建设阶段任务

### 验证结果
- 已人工核对章节与当前六层源码、46 个测试和 P1 基准的对应关系
- 一手资料优先使用原始论文、官方项目和官方文档
- 本阶段仅建立文档骨架，学习实验和链接自动校验尚未实现

### 下一步
1. 完成 01-04 基础 RAG 正文和离线实验
2. 为章节加入统一导航、折叠答案、源码定位和验收标准

### Git
- 提交：`31c00e8`
- 信息：`docs: 建立 agentRAG 系统学习课程骨架`

---

## 2026-07-15 22:32 | 系统学习课程：基础 RAG 01-04

### 当前目标
- 完成架构、文档处理、嵌入和精确向量检索四章
- 提供不下载模型即可复现的数据流实验，并将真实模型实验明确设为可选

### 完成内容
- 01 章讲解六层数据流、参数记忆与非参数记忆以及 index/ask/chat 调用链
- 02 章讲解字符分块、overlap、递归策略及边界反例
- 03 章讲解 Bi-Encoder、余弦相似度和当前近似 FIFO 缓存的真实行为
- 04 章讲解精确搜索、argpartition、内存估算、持久化和旧索引兼容
- 新增 `examples/learning/run_lab.py`，实现 01-04 快速离线实验
- 新增 `real_model_lab.py`，模型或依赖缺失时明确输出 `SKIPPED`

### 验证计划
- 依次运行 01-04 和 all 模式
- 验证真实生成模型缺失时只报告跳过，不使用 Mock 冒充真实推理

### 下一步
1. 完成 C++/pybind11、IVF-PQ、BM25、混合检索与重排章节
2. 扩展同一实验入口，避免复制生产实现

### Git
- 提交：`1ea29e5`
- 信息：`docs: 完成基础 RAG 学习章节与实验`

---

## 2026-07-15 23:05 | 系统学习课程：检索算法 05-08

### 当前目标
- 完成 C++/pybind11、K-Means、IVF-PQ、BM25、RRF 和重排课程
- 让实验直接复用当前 C++ 扩展、检索管道和 P1 基准

### 完成内容
- 05 章覆盖 Python/C++ 内存边界、K-Means 目标和 label 不稳定性
- 06 章覆盖残差 IVF-PQ、ADC、uint8 未位打包边界和 P1 低召回反例
- 07 章覆盖 BM25 公式、jieba 分词及 C++/Python 回退边界
- 08 章覆盖 RRF、候选池和 Cross-Encoder 未真实验证的状态
- 扩展课程实验 05-08；真实扩展缺失时明确 `SKIPPED`

### 验证计划
- 运行 05-08 和全量离线实验
- 确认 `n_probe` 实验输出召回与内存字段，混合检索能提升精确型号

### 下一步
1. 完成 Prompt、生成、KV 监控、Agent 和 CLI 章节
